...
component accessors="true" singleton {
	 ...
	 */
	Executor function newSingleExecutor ( required name, boolean debug = false, boolean loadAppContext = true )
	{
	 ...
	 */
	AsyncManager function shutdownExecutor ( required name, boolean force = false, numeric timeout = 30 )
	{
		if ( HasExecutor( arguments.name ) )
		{
			if ( arguments.force )
	 ...
	 */
	Future function newCompletedFuture ( required any value, boolean debug = false, boolean loadAppContext = true )
	{
		return new tasks.Future( argumentCollection = arguments );
	}
