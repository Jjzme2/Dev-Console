...
component accessors="true" {
	 ...
	 */
	any function get ( numeric timeout = 0, string timeUnit = "milliseconds", defaultValue )
	{
	...
	Future function exceptionally ( required target )
	{
		variables.native = variables.native.exceptionally(
			 CreateDynamicProxy(
				 new logbox.system.async.proxies.Function( arguments.target, variables.debug, variables.loadAppContext )
	 ...
	 */
	Future function run ( required supplier, method = "run", any executor = variables.executor )
	{
		var jSupplier = CreateDynamicProxy(
			 new logbox.system.async.proxies.Supplier(
				 arguments.supplier
				,arguments.method
				,variables.debug
	...
	Future function handle ( required action )
	{
		var biFunction = CreateDynamicProxy(
			 new logbox.system.async.proxies.BiFunction( arguments.action, variables.debug, variables.loadAppContext )
	...
	Future function whenComplete ( required action )
	{
		var biConsumer = CreateDynamicProxy(
			 new logbox.system.async.proxies.BiConsumer( arguments.action, variables.debug, variables.loadAppContext )
	...
	Future function thenRun ( required target )
	{
		var fConsumer = CreateDynamicProxy(
			 new logbox.system.async.proxies.Consumer( arguments.target, variables.debug, variables.loadAppContext )
	...
	Future function thenCompose ( required fn )
	{
		variables.native = variables.native.thenCompose(
			 CreateDynamicProxy(
				 new logbox.system.async.proxies.FutureFunction(
					 arguments.fn
					,variables.debug
					,variables.loadAppContext
				)
	...
	Future function thenCombine ( required future, fn )
	{
		variables.native = variables.native.thenCombine(
			...
			,CreateDynamicProxy(
				 new logbox.system.async.proxies.BiFunction( arguments.fn, variables.debug, variables.loadAppContext )
				,[ "java.util.function.BiFunction" ]
			)
		);
		return this;
	...
	)
	{
		...
		var jApply = CreateDynamicProxy(
			 new logbox.system.async.proxies.Function( arguments.fn, variables.debug, variables.loadAppContext )
			,[ "java.util.function.Function" ]
		);
