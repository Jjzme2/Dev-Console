...
component accessors="true" singleton {
	...
	ScheduledTask function task ( required name )
	{
		...
		variables.tasks[ arguments.name ] = {
			 // task name
			"name": arguments.name // task object
			,"task"        : oTask // task scheduled future object
			,"future"      : "" // when it registers
			,"registeredAt": Now() // when it's scheduled
			,"scheduledAt" : "" // Tracks if the task has been disabled for startup purposes
			,"disabled"    : false // If there is an error scheduling the task
			,"error"       : false // Any error messages when scheduling
			,"errorMessage": "" // The exception stacktrace if something went wrong scheduling the task
			,"stacktrace"  : "" // Server Host
