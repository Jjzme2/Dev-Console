...
component extends="coldbox.system.testing.BaseTestCase" appMapping="/" {
	...
	function run ()
	{
		Describe( "reminderHandler Suite", function ()
		{
			...
			It( "index", function ()
			{
				...
				var event = Get( "reminderHandler.index" );
			...
			{
				// Execute event or route via GET http method. Spice up accordingly
				var event = Get( "reminderHandler.view" );
				// expectations go here.
				Expect( false ).toBeTrue();
			} );
			...
			{
				// Execute event or route via GET http method. Spice up accordingly
				var event = Get( "reminderHandler.add" );
				// expectations go here.
				Expect( false ).toBeTrue();
			} );

			It( "markDone", function ()
			{
				// Execute event or route via GET http method. Spice up accordingly
				var event = Get( "reminderHandler.markDone" );
				// expectations go here.
				Expect( false ).toBeTrue();
			} );
			...
			{
				// Execute event or route via GET http method. Spice up accordingly
				var event = Get( "reminderHandler.edit" );
				// expectations go here.
				Expect( false ).toBeTrue();
			} );
		} );
