component {
	...
	function configure ()
	{
		...
		coldbox = {
			...
			,"eventName"               : "event" // Development Settings
			,"reinitPassword"          : ""
			,"reinitKey"               : "fwreinit"
			,"handlersIndexAutoReload" : true // Implicit Events
			,"defaultEvent"            : ""
			...
			,"sessionStartHandler"     : ""
			,"sessionEndHandler"       : ""
			,"missingTemplateHandler"  : "" // Extension Points
			...
			,"invalidEventHandler"     : ""
			,"customErrorTemplate"     : "" // Application Aspects
			,"handlerCaching"          : false
			,"eventCaching"            : false
			,"viewCaching"             : false // Will automatically do a mapDirectory() on your `models` for you.
			,"autoMapModels"           : true // Auto converts a json body payload into the RC
		...
		modules = {
			...
			"include": [] // An array of modules names to NOT load, empty means none
			,"exclude": []
		};
		...
		logBox = {
			 // Define Appenders
			"appenders": { "coldboxTracer": { "class": "coldbox.system.logging.appenders.ConsoleAppender" } } // Root Logger
			,"root": { "levelmax": "INFO", "appenders": "*" } // Implicit Level Categories
			,"info": [ "coldbox.system" ]
		};
		 ...
		 * --------------------------------------------------------------------------
		 */
		layoutSettings = { "defaultLayout": "customLayout", "defaultView": "" };
		...
		interceptors = [
			 {
				 "class": "interceptors.LayoutInterceptor"
				,"name" : "MyLayoutInterceptor"
				// ,'properties': { 'foo': 'bar', 'stuff': 'hello' }
			}
			,{
				 "class": "interceptors.HandlerInterceptor"
				,"name" : "MyHandlerInterceptor"
				//    ,'properties': { 'foo': 'bar', 'stuff': 'hello' }
			}
		];
		...
		moduleSettings = {
			 "cbmailServices": {
				...
				"tokenMarker"   : "@" // Default protocol to use, it must be defined in the mailers configuration
				,"defaultProtocol": "default" // Here you can register one or many mailers by name
				,"mailers"        : {
				...
				,"defaults"    : { "from": "John.zettler@mind-over-data.com", "cc": "" } // Whether the scheduled task is running or not
				,"runQueueTask": true
			}
		};
